...
class GenericMemberTypeProcessor<T extends HasDescription> extends SignatureVisitor {
    ...
    public void visitTypeArgument() {
        parameterizedType.addTypeArgument(new SignatureTypeArgumentProcessor.NewJavaTypeCreationProcess<>(new DomainBuilders.JavaWildcardTypeBuilder<T>()));
    }
    ...
    public void visitTypeVariable(String name) {
        typeCreationProcess = new SignatureTypeArgumentProcessor.ReferenceCreationProcess<>(name);
    }

    private void updateType(JavaParameterizedTypeBuilder<T> type) {
        this.parameterizedType = type;
        typeCreationProcess = new SignatureTypeArgumentProcessor.NewJavaTypeCreationProcess<>(this.parameterizedType, typeFinisher);
    }
}
